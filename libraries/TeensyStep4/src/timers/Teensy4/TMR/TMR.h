...
namespace TS4
{
    ...
    class TmrTimer : public ITimer
    {
     ...
     protected:
        static constexpr int prescale = 32;  // PCS = 0b1101 → IP bus clock ÷32 = 4.6875 MHz
        ...
        : regs(_regs)
    {
        ...
        // Leave CTRL=0 so start()'s "already enabled" guard (CM(1) check) does not
        // fire on first use.  start() sets CM(1) itself when it actually begins.
        regs->CTRL = 0x0000;
    }

    void TmrTimer::start()
    {
        // SAFETY: If the timer is already enabled, do not reset it!
        if (regs->CTRL & TMR_CTRL_CM(1)) {
            return; 
        }
        ...
        regs->CSCTRL = 0;
        regs->SCTRL  = 0;
        regs->CTRL = TMR_CTRL_CM(1) | TMR_CTRL_PCS(0b1101) | TMR_CTRL_LENGTH; // 0b1101 = IP bus ÷32
    ...
    {
        if (f <= 0) return; // Prevent division by zero

        float calcPeriod = (150E6 / (float)prescale) / f - pulsewidth - 1.5f;
        
        // CLAMP: Ensure period is at least 1 and doesn't exceed the 16-bit limit
        if (calcPeriod < 1.0f) calcPeriod = 1.0f;
        if (calcPeriod > 65535.0f) calcPeriod = 65535.0f;

        period = (uint16_t)calcPeriod;
    }
    ...
    void TmrTimer::setPulseParams(float width_us, unsigned stpPin)
    {
        ...
        this->pulsewidth              = width_us * (150.0f / (float)prescale) + 0.5;
        this->stpPin                  = stpPin;
