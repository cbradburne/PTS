...
def decode_cam_status(payload: bytes) -> dict:
    ...
    # Metadata parameter 10 carries the f-number as plain text — "f2.9", "f13.5"
    # — and arrives about 2.16s sooner than the APEX aperture above.  It is
    # DELIBERATELY NOT used as a source for f_stop.  That 2.16s is not the
    # camera answering sooner, it is the camera answering while the lens is
    # still moving: on 2026-08-15 a single move reported f14.5 at 22:55:54 and
    # f9.6 two seconds later, and the APEX value at 22:55:56.946 agreed with the
    # second.  Feeding both into one display therefore showed the aperture
    # change twice, once to a value the lens was merely passing through.
    #
    # The APEX report is late BECAUSE it is measured after the lens has settled,
    # which is what makes it the right one to show.  The text is still decoded
    # for the log, where watching the lens travel is informative rather than
    # distracting.
